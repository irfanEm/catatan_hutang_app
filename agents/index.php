...
$sortable = ['kode_agen', 'nama_agen', 'created_at'];
if (!in_array($sort, $sortable)) $sort = 'created_at';
$order = ($order === 'asc') ? 'asc' : 'desc';

// Hitung total data
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM agents WHERE deleted_at IS NULL AND (kode_agen LIKE :search OR nama_agen LIKE :search)");
...
$stmt = $pdo->prepare("SELECT * FROM agents 
  WHERE deleted_at IS NULL AND (kode_agen LIKE :search OR nama_agen LIKE :search) 
  ORDER BY $sort $order 
...
<div class="d-flex justify-content-between align-items-center mb-4">
  <h5 class="mb-0 fw-semibold"><i class="bi bi-person-lines-fill me-2"></i>Data Agen</h5>
  <a href="create.php" class="btn btn-sm btn-primary"><i class="bi bi-plus-circle me-1"></i> Tambah</a>
</div>

<form class="mb-3" method="GET">
  <div class="input-group input-group-sm">
    <input type="text" name="search" class="form-control" placeholder="Cari agen..." value="<?= htmlspecialchars($search) ?>">
...
<div class="table-responsive bg-white rounded shadow-sm p-3">
      ...
      <tbody>
        <?php foreach ($agents as $index => $agent): ?>
          <tr>
            ...
            <td class="text-end">
              <a href="edit.php?id=<?= $agent['id'] ?>" class="btn btn-sm btn-light border me-1" title="Edit"><i class="bi bi-pencil text-warning"></i></a>
              <a href="delete.php?id=<?= $agent['id'] ?>" class="btn btn-sm btn-light border" title="Hapus" onclick="return confirm('Hapus agen ini?')"><i class="bi bi-trash text-danger"></i></a>
            </td>
          </tr>
        <?php endforeach; ?>
      </tbody>
  ...
  <?php else: ?>
    <div class="alert alert-secondary text-center mb-0">
      <i class="bi bi-info-circle"></i> Tidak ada data agen saat ini.
    </div>
  <?php endif; ?>
</div>
